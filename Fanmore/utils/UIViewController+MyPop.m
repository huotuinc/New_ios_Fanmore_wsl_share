...
NSString const *MyPopupKey = @"MyPopupkey";
NSString const *MyBlurViewKey = @"MyFadeViewKey";
NSString const *MyUseBlurForPopup = @"MyUseBlurForPopup";

@implementation UIViewController (MyPop)
...
- (void)addBlurView {
    ...
    objc_setAssociatedObject(self, &MyBlurViewKey, blurView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)presentMyPopupViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion {
    if (self.popupViewController == nil) {
        ...
        CGRect finalFrame = [self getMyPopupFrameForViewController:viewControllerToPresent];
        ...
        if (self.useBlurForPopup) {
            ...
            objc_setAssociatedObject(self, &MyBlurViewKey, fadeView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        }
        UIView *blurView = objc_getAssociatedObject(self, &MyBlurViewKey);
        //
        //        [blurView bk_whenTapped:^{
        //            [self dismissPopupViewControllerAnimated:flag completion:NULL];
        //        }];
...
- (CGRect)getMyPopupFrameForViewController:(UIViewController *)viewController {
    CGRect frame = viewController.view.frame;
    CGFloat x;
    CGFloat y;
    if (UIDeviceOrientationIsPortrait([UIApplication sharedApplication].statusBarOrientation)) {
        x = ([UIScreen mainScreen].bounds.size.width - frame.size.width)/2;
        y = ([UIScreen mainScreen].bounds.size.height-64.0f - frame.size.height);
    } else {
        x = ([UIScreen mainScreen].bounds.size.height-64.0f - frame.size.width)/2;
...
- (void)screenOrientationChanged {
    ...
    [UIView animateWithDuration:ANIMATION_TIME animations:^{
        self.popupViewController.view.frame = [self getMyPopupFrameForViewController:self.popupViewController];
        if (UIDeviceOrientationIsPortrait([UIApplication sharedApplication].statusBarOrientation)) {
...
- (void)setPopupViewController:(UIViewController *)popupViewController {
    objc_setAssociatedObject(self, &MyPopupKey, popupViewController, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (UIViewController *)popupViewController {
    return objc_getAssociatedObject(self, &MyPopupKey);
...
- (void)setUseBlurForPopup:(BOOL)useBlurForPopup {
    if ([[[UIDevice currentDevice] systemVersion] floatValue] < 7.0 && useBlurForPopup) {
        ...
        objc_setAssociatedObject(self, &MyUseBlurForPopup, [NSNumber numberWithBool:NO], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    } else {
        objc_setAssociatedObject(self, &MyUseBlurForPopup, [NSNumber numberWithBool:useBlurForPopup], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
}

- (BOOL)useBlurForPopup {
    NSNumber *result = objc_getAssociatedObject(self, &MyUseBlurForPopup);
